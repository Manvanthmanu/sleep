...
def index(req):
    return render(req , './maths/index.html')

def analyzepage(req):
    return render(req,'./maths/Analyze.html' )
...
def calculation(req):
    ...
    # hello = chng
    fileNamee = '/manu.xlsx'
    pathname = 'maths/static/maths/data'+fileNamee
    
    writer2 = pd.ExcelWriter(pathname)
    ...
    executionpath=r'maths\static\maths\data'+fileNamee
    fileName= 'nature.xlsx'
    manu = pd.read_excel(executionpath , sheet_name = 'Day sleep')
    hello=pd.DataFrame.to_html(manu)

    params = {'heading': 'header' , 'data':hello  , 'pathgiver': executionpath }
    return render(req , 'maths/index2.html' , params)